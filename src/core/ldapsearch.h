...
public:
     ...
     * \a connection the connection used to construct LdapConnection object
     */
    ...
    /*!
     * Starts a search operation on the LDAP server \a server,
     * returning the attributes specified with \a attributes.
     * \a count means how many entries to list. If it's >0, then result()
